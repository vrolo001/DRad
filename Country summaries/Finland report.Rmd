...
drad_country <- drad_complete %>%
  filter(Residence == 6) #Finland = 6

nrow(drad_country) #n = 316
...
#write.table(org_table, file = "Finland org membership.txt", sep = ",", quote = FALSE, row.names = TRUE)
...
#write.table(polact_table, file = "Finland political actions.txt", sep = ",", quote = FALSE, row.names = TRUE)
```
...
favstats(PolTrust ~ Sex, data = drad_sex)
