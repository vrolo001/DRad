...
drad_country <- drad_complete %>%
  filter(Residence == 9) #Germany

nrow(drad_country) #n = 300

complete <- drad_complete %>%
  group_by(Residence) %>%
  summarise(n = n())

eights <- drad_complete %>%
  filter(PolAt_1 == 8) %>%
  group_by(Residence) %>%
  summarise(n = n())

cbind(eights[1], complete[2], eights[2], (eights[2]/complete[2] *100))
colSums(eights[2]) # n = 864 participants replied "dont know" to PolAt_1

```

# Missingness analyses

Judging by proportions of 8's to the PolAt_1 item by Residence, data do not seem to be missing at random. LSE workshop on data cleaning and screening does not seem to mention how to test for randomness of missing data when variables are categorical, but we can test if data are missing at random as far as the continuous data go. Below, only the variables in the path model are selected

```{r}
drad_model <- drad_complete %>%
  replace_with_na(replace = list(PolAt_1 = 8)) %>%
  select(SDO_T, OnlineGroupID, PolAt_1, Narcissism, Anomie, Deprivation_1, Deprivation_2, Cohesion, Alienation, Populism, RThreat, Extremism)
  
md.pattern(drad_model) #two patterns of data: complete responses, and responses mostly complete but with PolAt_1 NA (n = 864)
aggr(drad_model, col=c('white','red'), numbers = TRUE, sortVars = TRUE, cex.axis = .7, gap = 3, 
     ylab = c("Percentage of missing data","Missing Data Pattern")) #.19/19% missing data for PolAt_1

out <- TestMCARNormality(drad_model, del.lesscases = 1)
summary(out)

```

The MCAR test is a chi-square test which tests the null hypothesis of equality of covariances between missing and non-missing data groups. That is to say that the correlations between the variables in the cases with missing data are the same (equality) as the correlations between the variables in cases with no missing data. Because the p-value for the non-parametric test of homoscedasticity (equality of covariances) is smaller than .05, we reject the null hypothesis that the covariances are equal. They are not, thus data are not missing completely at random (MCAR)
...
#write.table(org_table, file = "Germany org membership.txt", sep = ",", quote = FALSE, row.names = TRUE)
...
#write.table(polact_table, file = "Germany political actions.txt", sep = ",", quote = FALSE, row.names = TRUE)
```
...
### Sex differences in political trust

```{r}
t.test(PolTrust ~ as.factor(Sex), data = drad_country, 
       var.equal = ifelse((leveneTest(PolTrust ~ as.factor(Sex), data = drad_country))$`Pr(>F)`[1] > .05, TRUE, FALSE) ) 

set.seed(828)
PolTrust.boot <- Boot(lm(PolTrust ~ Sex, data = drad_country), f = coef, R = 5000)
confint(PolTrust.boot, level = .95, type = "norm")

favstats(PolTrust ~ as.factor(Sex), data = drad_country)
```

### Sex differences in attitudes towards Russian migrants

```{r}
t.test(RusMigrant ~ as.factor(Sex), data = drad_country, 
       var.equal = ifelse((leveneTest(RusMigrant ~ as.factor(Sex), data = drad_country))$`Pr(>F)`[1] > .05, TRUE, FALSE)) 

set.seed(829)
RusMigrant.boot <- Boot(lm(RusMigrant ~ Sex, data = drad_country), f = coef, R = 5000)
confint(RusMigrant.boot, level = .95, type = "norm")

```

### Sex differences in attitudes towards Ukrainian migrants

```{r}
t.test(UkrMigrant ~ as.factor(Sex), data = drad_country, 
       var.equal = ifelse((leveneTest(UkrMigrant ~ as.factor(Sex), data = drad_country))$`Pr(>F)`[1] > .05, TRUE, FALSE)) 

set.seed(283)
UkrMigrant.boot <- Boot(lm(UkrMigrant ~ Sex, data = drad_country), f = coef, R = 5000)
