...
Until we get to the path model, any analyses conducted on one country will repeat on others countries. The code would thus be the same, and the only thing that changes is the dataset used. We could create several datasets called drad_*countryname*, but then this object would need to be changed across all the code when going from one country to another. A more practical option, is to assign a country's data to the same object (i.e., drad_country) and rerun the code. Each time a new country gets analysed, the only thing that needs to be changed is the *filter by Residence* command when defining drad_country.
...
#write.table(org_table, file = "Austria org membership.txt", sep = ",", quote = FALSE, row.names = TRUE)
...
#write.table(polact_table, file = "Austria political actions.txt", sep = ",", quote = FALSE, row.names = TRUE)
```
...
### Sex differences in online group identity

```{r}
(leveneTest(OnlineGroupID ~ as.factor(Sex), data = drad_sex))$`Pr(>F)`[1] > .05
t.test(OnlineGroupID ~ as.factor(Sex), data = drad_sex, 
       var.equal = ifelse((leveneTest(OnlineGroupID ~ as.factor(Sex), data = drad_sex))$`Pr(>F)`[1] > .05, TRUE, FALSE)
         ) 

set.seed(38)
OnlineGroupID.boot <- Boot(lm(OnlineGroupID ~ Sex, data = drad_sex), f = coef, R = 5000)
confint(OnlineGroupID.boot, level = .95, type = "norm")
```

### Sex differences in anomie

```{r}
t.test(Anomie ~ as.factor(Sex), data = drad_sex, 
       var.equal = ifelse((leveneTest(Anomie ~ as.factor(Sex), data = drad_sex))$`Pr(>F)`[1] > .05, TRUE, FALSE)
       ) 

set.seed(42)
Anomie.boot <- Boot(lm(Anomie ~ Sex, data = drad_sex), f = coef, R = 5000)
confint(Anomie.boot, level = .95, type = "norm")

favstats(Anomie ~ as.factor(Sex), data = drad_sex)
```
...
```{r}
t.test(PolTrust ~ as.factor(Sex), data = drad_sex, var.equal = 
         ifelse((leveneTest(PolTrust ~ as.factor(Sex), data = drad_sex))$`Pr(>F)`[1] > .05, TRUE, FALSE)
       ) 

set.seed(988)
PolTrust.boot <- Boot(lm(PolTrust ~ Sex, data = drad_sex), f = coef, R = 5000)
confint(PolTrust.boot, level = .95, type = "norm")

favstats(PolTrust ~ as.factor(Sex), data = drad_sex)
```

### Sex differences in realistic threat

```{r}
t.test(RThreat ~ as.factor(Sex), data = drad_sex, var.equal = 
        ifelse((leveneTest(RThreat ~ as.factor(Sex), data = drad_sex))$`Pr(>F)`[1] > .05, TRUE, FALSE)
       ) 
...
favstats(RThreat ~ as.factor(Sex), data = drad_sex)
```
...
set.seed(502)
dep2.boot <- Boot(lm(Deprivation_2 ~ discrim_group, data = drad_discrim), f = coef, R = 5000)
confint(dep2.boot, level = .95, type = "norm")
...
Because these regressions are mainly exploratory, perhaps bootstrapped CIs with listwise deletion is not necessary. As such, we keep all 292 cases for the time being and remove them only once we get to the path model
