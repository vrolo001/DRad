...
#write.table(org_table, file = "C:/Users/Vania/Desktop/DRad/data and code/R/DRad/tables/Poland org membership.txt", sep = ",", quote = FALSE, row.names = TRUE)
...
#write.table(polact_table, file = "C:/Users/Vania/Desktop/DRad/data and code/R/DRad/tables/Poland political actions.txt", sep = ",", quote = FALSE, row.names = TRUE)
```
...
### Sex differences in realistic threat

```{r}
t.test(RThreat ~ as.factor(Sex), data = drad_sex, 
       var.equal = ifelse((leveneTest(RThreat ~ as.factor(Sex), data = drad_sex))$`Pr(>F)`[1] > .05, TRUE, FALSE)) 

set.seed(372)
RThreat.boot <- Boot(lm(RThreat ~ Sex, data = drad_sex), f = coef, R = 5000)
confint(RThreat.boot, level = .95, type = "norm")

favstats(RThreat ~ Sex, data = drad_sex)

```

### Sex differences in extremism

```{r}
t.test(Extremism ~ as.factor(Sex), data = drad_sex, 
       var.equal = ifelse((leveneTest(Extremism ~ as.factor(Sex), data = drad_sex))$`Pr(>F)`[1] > .05, TRUE, FALSE)) 

set.seed(650)
Extremism.boot <- Boot(lm(Extremism ~ Sex, data = drad_sex), f = coef, R = 5000)
confint(Extremism.boot, level = .95, type = "norm")

favstats(Extremism ~ Sex, data = drad_sex)

```

### Sex differences in RusAtt

```{r}
t.test(RusAtt ~ as.factor(Sex), data = drad_sex, 
       var.equal = ifelse((leveneTest(RusAtt ~ as.factor(Sex), data = drad_sex))$`Pr(>F)`[1] > .05, TRUE, FALSE)) 

set.seed(714)
RusAtt.boot <- Boot(lm(RusAtt ~ Sex, data = drad_sex), f = coef, R = 5000)
confint(RusAtt.boot, level = .95, type = "norm")

favstats(RusAtt ~ Sex, data = drad_sex)

```

### Sex differences in RusMigrant
...
### Sex differences in UkrMigrant

```{r}
t.test(UkrMigrant ~ as.factor(Sex), data = drad_sex, 
       var.equal = ifelse((leveneTest(UkrMigrant ~ as.factor(Sex), data = drad_sex))$`Pr(>F)`[1] > .05, TRUE, FALSE)) 

set.seed(748)
UkrMigrant.boot <- Boot(lm(UkrMigrant ~ Sex, data = drad_sex), f = coef, R = 5000)
confint(UkrMigrant.boot, level = .95, type = "norm")

favstats(UkrMigrant ~ Sex, data = drad_sex)
...
```{r}
t.test(SDO_T ~ as.factor(discrim_group), data = drad_discrim, 
       var.equal = ifelse((leveneTest(SDO_T ~ as.factor(discrim_group), data = drad_discrim))$`Pr(>F)`[1] > .05, TRUE, FALSE)) 

set.seed(381)
SDO_T.boot <- Boot(lm(SDO_T ~ discrim_group, data = drad_discrim), f = coef, R = 5000)
confint(SDO_T.boot, level = .95, type = "norm")
...
```{r}
t.test(Narcissism ~ as.factor(discrim_group), data = drad_discrim, 
       var.equal = ifelse((leveneTest(Narcissism ~ as.factor(discrim_group), data = drad_discrim))$`Pr(>F)`[1] > .05, TRUE, FALSE)) 

set.seed(770)
Narcissism.boot <- Boot(lm(Narcissism ~ discrim_group, data = drad_discrim), f = coef, R = 5000)
confint(Narcissism.boot, level = .95, type = "norm")
...
favstats(Deprivation_1 ~ discrim_group, data = drad_discrim)
...
```{r}
t.test(Deprivation_2 ~ as.factor(discrim_group), data = drad_discrim, 
       var.equal = ifelse((leveneTest(Deprivation_2 ~ as.factor(discrim_group), data = drad_discrim))$`Pr(>F)`[1] > .05, TRUE, FALSE)) 

set.seed(179)
Deprivation_2.boot <- Boot(lm(Deprivation_2 ~ discrim_group, data = drad_discrim), f = coef, R = 5000)
confint(Deprivation_2.boot, level = .95, type = "norm")

favstats(Deprivation_2 ~ discrim_group, data = drad_discrim)
...
favstats(Alienation ~ discrim_group, data = drad_discrim)
...
```{r}
t.test(RThreat ~ as.factor(discrim_group), data = drad_discrim, 
       var.equal = ifelse((leveneTest(RThreat ~ as.factor(discrim_group), data = drad_discrim))$`Pr(>F)`[1] > .05, TRUE, FALSE)) 

set.seed(457)
RThreat.boot <- Boot(lm(RThreat ~ discrim_group, data = drad_discrim), f = coef, R = 5000)
confint(RThreat.boot, level = .95, type = "norm")
...
```{r}
t.test(UkrMigrant ~ as.factor(discrim_group), data = drad_discrim, 
       var.equal = ifelse((leveneTest(UkrMigrant ~ as.factor(discrim_group), data = drad_discrim))$`Pr(>F)`[1] > .05, TRUE, FALSE)) 

set.seed(182)
UkrMigrant.boot <- Boot(lm(UkrMigrant ~ discrim_group, data = drad_discrim), f = coef, R = 5000)
confint(UkrMigrant.boot, level = .95, type = "norm")

favstats(UkrMigrant ~ discrim_group, data = drad_discrim)
