...
#write.table(org_table, file = "C:/Users/Vania/Desktop/DRad/data and code/R/DRad/tables/Kosovo org membership.txt", sep = ",", quote = FALSE, row.names = TRUE)
...
#write.table(polact_table, file = "C:/Users/Vania/Desktop/DRad/data and code/R/DRad/tables/Kosovo political actions.txt", sep = ",", quote = FALSE, row.names = TRUE)
```
...
### Sex differences in attitudes against Russia
...
```{r}
t.test(Anomie ~ as.factor(discrim_group), data = drad_discrim, 
       var.equal = ifelse((leveneTest(Anomie ~ as.factor(discrim_group), data = drad_discrim))$`Pr(>F)`[1] > .05, TRUE, FALSE)) 

set.seed(65) 
Anomie.boot <- Boot(lm(Anomie ~ discrim_group, data = drad_discrim), f = coef, R = 5000)
confint(Anomie.boot, level = .95, type = "norm")

favstats(Anomie ~ as.factor(discrim_group), data = drad_discrim)
...
t.test(Deprivation_1 ~ as.factor(discrim_group), data = drad_discrim, 
       var.equal = ifelse((leveneTest(Deprivation_1 ~ as.factor(discrim_group), data = drad_discrim))$`Pr(>F)`[1], TRUE, FALSE)) 
...
```{r}
t.test(Populism ~ as.factor(discrim_group), data = drad_discrim, 
       var.equal = ifelse((leveneTest(Populism ~ as.factor(discrim_group), data = drad_discrim))$`Pr(>F)`[1] > .05, TRUE, FALSE)) 

set.seed(580)
Populism.boot <- Boot(lm(Populism ~ discrim_group, data = drad_discrim), f = coef, R = 5000)
