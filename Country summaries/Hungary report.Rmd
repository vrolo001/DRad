...
drad_country <- drad_complete %>%
  filter(Residence == 10) #Hungary = 10

nrow(drad_country) #n = 318
...
#write.table(org_table, file = "C:/Users/Vania/Desktop/DRad/data and code/R/DRad/tables/Hungary org membership.txt", sep = ",", quote = FALSE, row.names = TRUE)
...
#write.table(polact_table, file = "C:/Users/Vania/Desktop/DRad/data and code/R/DRad/tables/Hungary political actions.txt", sep = ",", quote = FALSE, row.names = TRUE)
```
...
### Sex differences in SDO_T

```{r}
t.test(SDO_T ~ as.factor(Sex), data = drad_sex, 
       var.equal = ifelse((leveneTest(SDO_T ~ as.factor(Sex), data = drad_sex))$`Pr(>F)`[1] > .05, TRUE, FALSE)) 

set.seed(164)
SDO_T.boot <- Boot(lm(SDO_T ~ Sex, data = drad_sex), f = coef, R = 5000)
confint(SDO_T.boot, level = .95, type = "norm")

favstats(SDO_T ~ Sex, data = drad_sex)
...
favstats(SDO_T ~ as.factor(discrim_group), data = drad_discrim)
...
```{r}
t.test(OnlineGroupID ~ as.factor(discrim_group), data = drad_discrim, 
       var.equal = ifelse((leveneTest(OnlineGroupID ~ as.factor(discrim_group), data = drad_discrim))$`Pr(>F)`[1] > .05, TRUE, FALSE)) 

set.seed(695)
OnlineGroupID.boot <- Boot(lm(OnlineGroupID ~ discrim_group, data = drad_discrim), f = coef, R = 5000)
confint(OnlineGroupID.boot, level = .95, type = "norm")

favstats(OnlineGroupID ~ as.factor(discrim_group), data = drad_discrim)
...
favstats(Narcissism ~ as.factor(discrim_group), data = drad_discrim)
