...
#write.table(org_table, file = "C:/Users/Vania/Desktop/DRad/data and code/R/DRad/tables/Turkey org membership.txt", sep = ",", quote = FALSE, row.names = TRUE)
...
#write.table(polact_table, file = "C:/Users/Vania/Desktop/DRad/data and code/R/DRad/tables/Turkey political actions.txt", sep = ",", quote = FALSE, row.names = TRUE)
```
...
### Sex differences in alienation

```{r}
t.test(Alienation ~ as.factor(Sex), data = drad_sex, 
       var.equal = ifelse((leveneTest(Alienation ~ as.factor(Sex), data = drad_sex))$`Pr(>F)`[1] > .05, TRUE, FALSE)) 

set.seed(901)
Alienation.boot <- Boot(lm(Alienation ~ Sex, data = drad_sex), f = coef, R = 5000)
...
favstats(Populism ~ as.factor(discrim_group), data = drad_discrim)
