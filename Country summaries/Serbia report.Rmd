...
#write.table(org_table, file = "C:/Users/Vania/Desktop/DRad/data and code/R/DRad/tables/Serbia org membership.txt", sep = ",", quote = FALSE, row.names = TRUE)
...
#write.table(polact_table, file = "C:/Users/Vania/Desktop/DRad/data and code/R/DRad/tables/Serbia political actions.txt", sep = ",", quote = FALSE, row.names = TRUE)
```
...
favstats(Anomie ~ Sex, data = drad_sex)

```

### Sex differences in GRD

```{r}
t.test(Deprivation_2 ~ as.factor(Sex), data = drad_sex, 
       var.equal = ifelse((leveneTest(Deprivation_2 ~ as.factor(Sex), data = drad_sex))$`Pr(>F)`[1] > .05, TRUE, FALSE)) 

set.seed(937)
Deprivation_2.boot <- Boot(lm(Deprivation_2 ~ Sex, data = drad_sex), f = coef, R = 5000)
confint(Deprivation_2.boot, level = .95, type = "norm")

favstats(Deprivation_2 ~ Sex, data = drad_sex)

```

### Sex differences in social cohesion

```{r}
t.test(Cohesion ~ as.factor(Sex), data = drad_sex, 
       var.equal = ifelse((leveneTest(Cohesion ~ as.factor(Sex), data = drad_sex))$`Pr(>F)`[1] > .05, TRUE, FALSE)) 

set.seed(387)
Cohesion.boot <- Boot(lm(Cohesion ~ Sex, data = drad_sex), f = coef, R = 5000)
confint(Cohesion.boot, level = .95, type = "norm")

favstats(Cohesion ~ Sex, data = drad_sex)
...
favstats(RThreat ~ as.factor(discrim_group), data = drad_discrim)
