...
#write.table(org_table, file = "C:/Users/Vania/Desktop/DRad/data and code/R/DRad/tables/Israel org membership.txt", sep = ",", quote = FALSE, row.names = TRUE)
...
#write.table(polact_table, file = "C:/Users/Vania/Desktop/DRad/data and code/R/DRad/tables/Israel political actions.txt", sep = ",", quote = FALSE, row.names = TRUE)
```
...
```{r}
t.test(PolTrust ~ as.factor(Sex), data = drad_sex, 
       var.equal = ifelse((leveneTest(PolTrust ~ as.factor(Sex), data = drad_sex))$`Pr(>F)`[1] > .05, TRUE, FALSE)) 
...
set.seed(1)
SDO.boot <- Boot(lm(SDO_T ~ discrim_group, data = drad_discrim), f = coef, R = 5000)
confint(SDO.boot, level = .95, type = "norm")
...
```{r}
t.test(Cohesion ~ as.factor(discrim_group), data = drad_discrim, 
       var.equal = ifelse((leveneTest(Cohesion ~ as.factor(discrim_group), data = drad_discrim))$`Pr(>F)`[1] > .05, TRUE, FALSE)) 

set.seed(9355)
Cohesion.boot <- Boot(lm(Cohesion ~ discrim_group, data = drad_discrim), f = coef, R = 5000)
